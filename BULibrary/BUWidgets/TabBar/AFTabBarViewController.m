...
- (void)SelectViewControllerItem:(AFTabBarItem *)argBtn
{
    ...
    if (m_pSelectedBtn != argBtn)
    {
            ...
            {
                [self ShowTabBar];
                if (IS_EMPTY_STRING(argBtn.currentTitle)) {
                    [[pCurrentNVC topViewController].view addSubview:m_pTabBarBgImageV];
                    [[pCurrentNVC topViewController].view sendSubviewToBack:m_pTabBarBgImageV];
                }
            }
    }
    else
    {
        [[NSNotificationCenter defaultCenter]postNotificationName:kItemRefreshNotifity object:@(argBtn.tag)];
    }
}
